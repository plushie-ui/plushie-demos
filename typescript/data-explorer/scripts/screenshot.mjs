/**
 * Take a headless screenshot and save as PNG.
 */
...
if (result.rgba && (result.rgba instanceof Uint8Array || Buffer.isBuffer(result.rgba))) {
  const rgbaPath = `${dir}/data-explorer.rgba`
  writeFileSync(rgbaPath, result.rgba)
  execSync(`convert -size ${result.width}x${result.height} -depth 8 rgba:${rgbaPath} ${dir}/data-explorer.png`)
  console.log(`Saved: ${dir}/data-explorer.png`)
}

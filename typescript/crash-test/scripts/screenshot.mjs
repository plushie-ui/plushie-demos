...
if (result.rgba && (result.rgba instanceof Uint8Array || Buffer.isBuffer(result.rgba))) {
  const rgbaPath = `${dir}/crash-test.rgba`
  writeFileSync(rgbaPath, result.rgba)
  execSync(`convert -size ${result.width}x${result.height} -depth 8 rgba:${rgbaPath} ${dir}/crash-test.png`)
  console.log(`Saved: ${dir}/crash-test.png`)
}

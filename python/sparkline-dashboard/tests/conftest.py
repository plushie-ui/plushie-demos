...
def _has_extension_binary() -> bool:
    """Check whether a custom-built binary with the sparkline extension exists.
...
def needs_extension_binary() -> None:
    """Skip the calling test if the sparkline extension binary is not available."""
    if not _has_extension_binary():
        pytest.skip(
            "sparkline extension binary not built -- run: python -m plushie build"
        )
